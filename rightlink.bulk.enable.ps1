...
'@
    ...
    if ((!$instanceHref) -and (!$cloudType)) {
      GetHelp
      LogError "ERROR: -CloudType and -InstanceHref missing. At least one must be present."
    }
    ...
    $RIGHTLINK_INST_DIR = "C:\Windows\Temp\RightLinkInstaller"
      ...
      if ($cloudType -eq 'amazon') {
        ...
        if (!$instanceId) {
          ...
          if ($instanceId -match "^10\.|^192\.168\.|^172\.(1[6-9]|2[0-9]|3[0-1])\.") {
            LogWrite "Using a private IP address as a unique identifier is not recommended."
            LogWrite "Only use this method if you are sure you do not have any overlapping IP space in any vScale clouds."
            $response = Read-Host "Continue using the private IP address as a unique identifier? [y/N]"
    ...
    # Theres two different formats (RL and RLL). Ensure unformity by translating
    # them both into RLL format.
    $RS_VARS = ""
